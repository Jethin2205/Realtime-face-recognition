...
def put_text_bg(frame, text, pos, scale=0.55, fg=(255,255,255), bg=(20,20,20)):
    ...
    x, y = pos
    # clamp so text never goes off screen
    h, w = frame.shape[:2]
    x = max(0, min(x, w - tw - 6))
    y = max(th + 3, min(y, h - bl - 3))
...
def draw_faces(frame, faces, analyses):
    ...
    for face, a in zip(faces, analyses):
        ...
        put_text_bg(out, label, (face.left, face.top - 10), fg=color)

        # attributes inside box (top-left corner so always visible)
        put_text_bg(out, f"Age: {a.age}",              (face.left + 4, face.top + 20), fg=(200,230,255))
        put_text_bg(out, f"Gender: {a.gender}",        (face.left + 4, face.top + 42), fg=(255,210,180))
        put_text_bg(out, f"Emotion: {a.emotion} {a.emotion_conf:.0%}", (face.left + 4, face.top + 64), fg=emo_c)
    return out
